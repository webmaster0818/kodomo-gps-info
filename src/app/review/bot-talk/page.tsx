...
const faqData = [
  { q: 'BoTトークの口コミを読むときのポイントは？', a: 'BoTトークはGPSプラン(月額528円)とトークプラン(748円)で使える機能が大きく違うため、口コミがどちらのプランの感想かを確認するのが最重要ポイントです。またAI学習型の見守り(行動を学習して通知)という方式のため、使い始め直後の精度に関する口コミは学習前の評価である可能性があります。' },
  { q: 'BoTトークは海外でも使えますか？', a: '使えません。BoT端末の位置情報送信は日本国内のLTE網を利用しているため、端末を海外に持って行っても利用できないと公式サポートに明記されています（2026年9月5日確認）。ただし「お子さまが日本国内・見守る保護者が海外」というケースは、保護者側がインターネットに接続できれば位置確認が可能です（単身赴任・海外出張のご家庭でも使えます）。' },
